...
def _get_course_request(course_id, user):
    rf = RequestFactory()
    request = rf.get(reverse('get-course'), args=[course_id])
    request.user = user
    response = ajax_request_handlers.GetCourse.as_view()(request)

    return _bytes_to_json(response.content)
...
class CourseRequestHandlerTestCase(TemporaryFilesystemTestCase):
    ...
    def test_get_course_as_admin(self):
        ...
        self.courses[2].add_course_admins(course_admin)
