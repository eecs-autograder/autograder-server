...
class RerunSubmissionsTaskTestCase(UnitTestBase):
    ...
    def test_default_create(self):
        ...
        self.assertTrue(rerun_task.rerun_all_mutation_test_suites)
        self.assertEqual([], rerun_task.mutation_suite_pks)
    ...
    def test_create_non_defaults(self):
        ...
        self.assertFalse(rerun_task.rerun_all_mutation_test_suites)
        self.assertEqual([self.mutation_test_suite.pk], rerun_task.mutation_suite_pks)
    ...
    def test_progress_computation(self):
        ...
        num_subtasks = (self.project.ag_test_suites.count()
                        + self.project.mutation_test_suites.count())
    ...
    def test_error_some_mutation_suites_not_in_project(self):
        other_mutation_suite = obj_build.make_mutation_test_suite()
        ...
        with self.assertRaises(exceptions.ValidationError) as cm:
            ag_models.RerunSubmissionsTask.objects.validate_and_create(
                ...
                project=self.project,
                rerun_all_mutation_test_suites=False,
                mutation_suite_pks=[other_mutation_suite.pk])

        self.assertIn('mutation_suite_pks', cm.exception.message_dict)
