...
class ListCourseHandgradersTestCase(_HandgradersSetUp,
    ...
    def test_admin_list_handgraders(self):
        expected_content = ag_serializers.UserSerializer(
            self.handgraders, many=True).data

        self.do_list_objects_test(
            self.client, self.admin, self.url, expected_content)
...
class AddCourseHandgradersTestCase(_HandgradersSetUp, UnitTestBase):
    ...
    def test_admin_add_handgraders(self):
        ...
        self.assertEqual(len(self.current_handgraders),
                         self.course.handgraders.count())

        response = self.client.patch(
            self.url, {'new_handgraders': new_handgrader_names})
    ...
    def test_add_handgraders_permission_denied(self):
        for user in self.staff, self.current_handgraders[0], self.nobody:
            ...
            response = self.client.patch(
                self.url, {'new_handgraders': ['fake_name']})
            self.assertEqual(status.HTTP_403_FORBIDDEN, response.status_code)

            self.assertCountEqual(self.current_handgraders,
                                  self.course.handgraders.all())
